...
interface EditPatientPlanModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
...
export default function EditPatientPlanModal({ isOpen, onClose, onSuccess, patientPlan }: EditPatientPlanModalProps) {
  const [planType, setPlanType] = useState<'recorrente' | 'sessoes'>(patientPlan?.plan_type || 'recorrente');
  ...
  useEffect(() => {
    if (patientPlan) {
      reset({
        ...
        plan_type: patientPlan.plan_type,
        sessions_monthly: patientPlan.sessions_monthly || 1,
        sessions_max: patientPlan.sessions_max || 1,
        price: patientPlan.price,
        status: patientPlan.status,
        notes: patientPlan.notes || '',
      });
      setPlanType(patientPlan.plan_type);
    }
  }, [patientPlan, reset]);
  ...
  async function onSubmit(data: PatientPlanFormData) {
    try {
      ...
      if (data.plan_type === 'recorrente') {
        updateData.sessions_monthly = parseInt(data.sessions_monthly?.toString() || '1');
        delete updateData.sessions_max;
      } else {
        updateData.sessions_max = parseInt(data.sessions_max?.toString() || '1');
        delete updateData.sessions_monthly;
      }

      await patientPlanService.updateById(patientPlan.id, updateData);

      showToast.success('Plano de paciente atualizado com sucesso!');
      onClose();
      if (onSuccess) onSuccess();
    } catch (error: any) {
      showToast.error(error.message || 'Erro ao atualizar plano de paciente');
    }
  }

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Editar Plano de Paciente" size="md">
      <div className="flex items-center gap-2 mb-4 text-lg font-semibold">
        <PencilSimple size={20} /> Editar Plano de Paciente
      </div>
