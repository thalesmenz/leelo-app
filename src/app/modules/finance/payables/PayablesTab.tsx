    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-6 lg:p-8">
        <div className="flex items-center justify-center h-32">
          <div className="text-sm sm:text-base text-gray-500">Carregando contas a pagar...</div>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4 sm:mb-6">
        <div className="flex items-center gap-2 text-lg sm:text-xl font-bold text-blue-600">
          <CurrencyDollar size={20} className="sm:w-6 sm:h-6" /> Contas a Pagar
        </div>
        <button 
        ...
        <>
          ...
          <div className="md:hidden space-y-3">
            {getFilteredPayables().map((payable) => (
              <div key={payable.id} className="border border-gray-200 rounded-lg p-4 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-sm sm:text-base text-gray-900 mb-1 truncate">{payable.name}</h3>
                    <p className="text-xs sm:text-sm text-gray-600 mb-2 line-clamp-2">{payable.description || '-'}</p>
                    <div className="flex flex-col gap-1.5 mb-2">
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        <CalendarBlank size={14} />
                        <span>Vencimento: <span className={isOverdue(payable.due_date) ? 'text-red-600 font-semibold' : ''}>{formatDate(payable.due_date)}</span></span>
                      </div>
                      {isConsolidatedView && (
                        <div className="flex items-center gap-1 text-xs text-gray-500">
                          <User size={14} />
                          <span className="truncate">{payable.user?.name || 'Usuário não identificado'}</span>
                        </div>
                      )}
                    </div>
                  </div>
