    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-8">
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${statusFilter === 'todos' ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
        >
          Todos ({patients.length})
        </button>
        <button
          onClick={() => setStatusFilter('Ativo')}
          className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${statusFilter === 'Ativo' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
        >
          Ativos ({patients.filter(p => p.status === 'Ativo').length})
        </button>
        <button
          onClick={() => setStatusFilter('Inativo')}
          className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${statusFilter === 'Inativo' ? 'bg-red-100 text-red-700 border border-red-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
        >
